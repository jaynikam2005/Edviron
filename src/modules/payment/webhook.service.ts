...
export class WebhookService {
  ...
  async processWebhook(payload: WebhookPayloadDto): Promise<WebhookResponseDto> {
    ...
    try {
      ...
      if (!order) {
        this.logger.warn(`Order not found for webhook: ${JSON.stringify(payload.order_info)}`);
  ...
  private async findOrder(orderInfo: any): Promise<OrderDocument | null> {
    ...
    if (orderInfo.custom_order_id) {
      const order = await this.orderModel.findOne({ custom_order_id: orderInfo.custom_order_id }).exec();
      if (order) return order;
    }

    // Try to find by order_id as custom_order_id
    if (orderInfo.order_id) {
      const order = await this.orderModel.findOne({ custom_order_id: orderInfo.order_id }).exec();
      if (order) return order;
    }
  ...
  private async updateOrderStatus(order: OrderDocument, payload: WebhookPayloadDto): Promise<void> {
    ...
    const existingOrderStatus = await this.orderStatusModel.findOne({ collect_id: order._id }).exec();
    ...
    if (existingOrderStatus) {
      ...
      await this.orderStatusModel.findByIdAndUpdate(
        ...
        {
          ...orderStatusData,
          error_message: payload.status === 'failed' ? payload.gateway_response : undefined,
        },
        { new: true }
      ).exec();
      
      this.logger.log(`Updated existing OrderStatus: ${existingOrderStatus._id}`);
    } else {
      // Create new record
  ...
  async getWebhookLogsByStatus(processingStatus: string): Promise<WebhookLogs[]> {
    return this.webhookLogsModel
